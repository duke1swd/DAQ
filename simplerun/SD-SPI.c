    ...
    const rom typMMC_CMD sdmmc_cmdtable[] =
{
    ...
    {cmdERASE,                  0xFF,   R1b,    NODATA},
    {cmdAPP_CMD,		0xFF,	R1,	NODATA},
...
MMC_RESPONSE SendMMCCmd(BYTE cmd, DWORD address)
{
    ...
    if(sdmmc_cmdtable[cmd].responsetype == R1b)
    {
        ...
        for(index =0; index < 0xFF && dataresponse == 0x00; index++)
        {
            timeout = 0xFFFF;
            
            do
            {
                dataresponse = MDD_SDSPI_ReadMedia();
                timeout--;
            }while(dataresponse == 0x00 && timeout != 0);
        }
    }
    ...
    if (SendStopOnError && response.r1._byte != 0)
    	blink_error_code(15, cmd, response.r1._byte);
...
near WORD SW_index;
near BYTE SW_data_response;
near BYTE SW_clear;
near MMC_RESPONSE SW_response; 
#pragma udata

static BYTE
send_block(BYTE *buffer)
{
    /*
     * Loop sending 512 bytes of data
     */
    for(SW_index = 0; SW_index < MEDIA_SECTOR_SIZE; ) {
        SW_clear = SPIBUF;
        SPI_INTERRUPT_FLAG = 0;
        SPIBUF = buffer[SW_index++];   // write byte to SSP1BUF register
    ...
    SW_clear = SPIBUF;         // Clear the SPIBUF

    // calc crc
    mSendCRC();                                 //Send 2 bytes of CRC
    
    SW_data_response = MDD_SDSPI_ReadMedia();      //Read response
    
    if((SW_data_response & 0x0F) != DATA_ACCEPTED) {
	SD_CS = 1;
	return FALSE;
    }
    return TRUE;
...
BYTE MDD_SDSPI_SectorWriteStart(DWORD sector_addr, BYTE* buffer)
{
    if (sector_addr == 0) {
/*xxx*/blink_error(13);
    	return FALSE;
    ...
    // send the cmd
    SW_response = SendMMCCmd(WRITE_SINGLE_BLOCK,(sector_addr << 9));
    
    // see if it was accepted
    if(SW_response.r1._byte != 0x00) {
	SD_CS = 1;
/*xxx*/blink_error_code(3, 1, SW_response.r1._byte);
	return FALSE;
    }

    WriteSPIM(DATA_START_TOKEN);                 //Send data start token

    return send_block(buffer);
}

BYTE MDD_SDSPI_MultiWriteBlockFinish(void)
{
    SW_index = 0;
    ...
    do                  //Wait for write completion
    {
        ...
        SPIBUF = 0xFF;
        while(!SPI_INTERRUPT_FLAG);
        SW_data_response = SPIBUF;
        SW_index++;
    }while((SW_data_response == 0x00) && (SW_index != 0));

    if(SW_index == 0)  {           //if timeout first
        SD_CS = 1;
        return FALSE;
    }
...
BYTE MDD_SDSPI_SectorWriteFinish(void)
{
    BYTE status;
    
    status = MDD_SDSPI_MultiWriteBlockFinish();
    SD_CS = 1;

    return status;
}

BYTE MDD_SDSPI_MultiWriteStart(DWORD sector_addr)
{
    if (sector_addr == 0)
    	return FALSE;

    // send the cmd
    SW_response = SendMMCCmd(WRITE_MULTI_BLOCK,(sector_addr << 9));
...
BYTE MDD_SDSPI_MultiWriteFinish(void)
{
	// Tell the card we are done.
	WriteSPIM(DATA_MULT_WRT_STOP_TOK);

	// Wait for complete.
	return MDD_SDSPI_SectorWriteFinish();
}

BYTE MDD_SDSPI_MultiWriteBlockStart(BYTE *buffer)
{
    WriteSPIM(DATA_MULT_WRT_START_TOK);      //Send multiblock data start token
...
BYTE MDD_SDSPI_SectorWrite(DWORD sector_addr, BYTE* buffer, BYTE allowWriteToZero)
{
    if (!MDD_SDSPI_SectorWriteStart(sector_addr, buffer))
    	return FALSE;
...
/*
 * Set the erase block length.
 */
BYTE MDD_SDSPI_EraseBlockLen(BYTE block_len)
{
    MMC_RESPONSE    response; 
    DWORD           ebl;
    BYTE            status = TRUE;
    ...
    response = SendMMCCmd(APP_CMD, 0);
    if(response.r1._byte != 0x00)
        status = FALSE;

    response = SendMMCCmd(SET_ERASE_BLOCK_LEN, ebl);
    if(response.r1._byte != 0x00)
        status = FALSE;
...
/*****************************************************************************
  Function:
    BYTE MDD_SDSPI_Erase (DWORD sector_addr_start, DWORD sector_addr_end)
  Summary:
    Erase a set of sectors.
  Conditions:
    None.
  Input:
    sector_addr_start -      The address of the first sector to erase.
    sector_addr_end -        The address of the last sector to erase.
  Return Values:
    TRUE -  Erased
    FALSE - Error of some sort
  Side Effects:
    None.
  Description:
    Erases the range specified.  May take a while.
  Remarks:
    The card expects the address field in the command packet to be
    a byte address.  The sector_addr value is converted to a byte
    address by shifting it left nine times (multiplying by 512).
    Note: don't give this too big a range, or the erase might time out.
  ***************************************************************************************/
...
BYTE MDD_SDSPI_Erase (DWORD sector_addr_start, DWORD sector_addr_end)
{
    ...
    // Mark the start of the erase region.
    response = SendMMCCmd(TAG_SECTOR_START, sector_addr_start);
    if(response.r1._byte != 0x00) {
        return FALSE;
    }

    // Mark the end of the erase region.
    response = SendMMCCmd(TAG_SECTOR_END, sector_addr_end);
    if(response.r1._byte != 0x00) {
        return FALSE;
    }

    // Doit
    response = SendMMCCmd(ERASE, 0);
    if(response.r1._byte != 0x00) {
        return FALSE;
    }
