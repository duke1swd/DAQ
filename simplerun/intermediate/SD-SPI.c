 ...
 * NOTES ON RING MODE
 *
 * The ring mode allows for tight coordination between data producers
 * and the WriteSector routine.  Each record must be a multiple of 2 in
 * size.  The first byte of a record that is ready to write must be
 * non zero.  Once the record has been written the SectorWrite routine
 * will reset that byte to zero.
 */

// comment out this define to remove the feature.
//#define	RING_RECORD_SIZE	8	// in bytes.  MUST BE POWER OF 2

#ifdef RING_RECORD_SIZE
#define	RING_MASK		(RING_RECORD_SIZE - 1)
#endif
...
BYTE MDD_SDSPI_SectorWrite(DWORD sector_addr, BYTE* buffer, BYTE allowWriteToZero)
{
    ...
    else
    {
        ...
        else
        {
            ...
            for(SW_index = 0; SW_index < MEDIA_SECTOR_SIZE; SW_index++)      //Send 512 bytes of data
            {
                SW_clear = SPIBUF;
                SPI_INTERRUPT_FLAG = 0;
...
#ifdef RING_MASK
		/*
		 * If ring-buffer mode is enabled then
		 * wait until the next record is ready before we write it.
		 */
		//if ((SW_index & RING_MASK) == 0) {
			ring_zero_p = buffer + SW_index;
			while (*ring_zero_p == 0)
				;
		//}
#endif
		...
		SW_index++;
                ...
                SPIBUF = buffer[SW_index];     // write byte to SSP1BUF register
                while( !SPI_INTERRUPT_FLAG );  // wait until bus cycle complete
		// don't increment index here.  for statement does it.
                SW_clear = SPIBUF;

#ifdef RING_MASK
		/*
		 * If ring-buffer mode is enabled and we've just written
		 * the last byte of a record, mark the record as written.
		 */
		//if ((SW_index & RING_MASK) == RING_MASK)
			*ring_zero_p = (BYTE)0;
#endif
            }
            ...
            {
                SW_index = 0;            //using i as a timeout counter
