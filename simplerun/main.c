...
#define	N_BUFFERS	3
#define	SAMPLE_MODE_1

#ifdef SAMPLE_MODE_1
#define	SAMPLES_PER_SECT	255
#endif

#ifdef SAMPLE_MODE_3
#define	SAMPLES_PER_SECT	102
#endif
...
adc_ring_init(void)
{
	...
	bn = 0;
	record_p = buffer;
	samples_to_fill = SAMPLES_PER_SECT;
...
adc_isr(void)
{
	 ...
	 * Get the data.
	 *
	 * DO NOT PUT ANYTHING ABOVE THESE LINES OF CODE.
	 */
	adc_channel = 5;	/* xxx QQQ xxx get the unfiltered pin */
	adc_sample();
	 ...
	 */
	if (samples_to_fill == SAMPLES_PER_SECT) {
		if ((bn == 0 && b1flag == 1) ||
		    (bn == 1 && b2flag == 1) ||
		    (bn == 2 && b3flag == 1)) {
		    	adc_sample_skip++;
...
/*
 * Differen modes of erase.  Define at most one of these.
 */
//#define	SHORT_ERASE	32	// in sectors
#define	MULTIBLOCK_LEN	NSECT	// in sectors.

#ifdef SHORT_ERASE
static unsigned int sectors_before_erase;
...
maybe_erase(void)
{
	...
	if (sectors_before_erase) {
		sectors_before_erase -= 1;
		return;
	}
	sectors_before_erase = SHORT_ERASE - 1;

	r = MDD_SDSPI_Erase(lba + 0, lba + (SHORT_ERASE-1));
	if (!r)
		blink_error(7);
}
#endif

#ifdef MULTIBLOCK_LEN
static unsigned unsigned int sectors_left_in_multiblock;

static void
multi_init(void)
{
	sectors_left_in_multiblock = 0;
}
...
multi_start(BYTE *bp)
{
	...
	if (sectors_left_in_multiblock == 0) {
		sectors_left_in_multiblock = MULTIBLOCK_LEN;

		r = MDD_SDSPI_EraseBlockLen(MULTIBLOCK_LEN);
		if (r == FALSE)
			blink_error(9);

		r = MDD_SDSPI_MultiWriteStart(lba);
		if (r == FALSE)
			blink_error(10);
	}

	r = MDD_SDSPI_MultiWriteBlockStart(bp);
	if (r == FALSE)
		blink_error(11);
}
...
multi_finish(void)
{
	...
	r = MDD_SDSPI_MultiWriteBlockFinish();
	if (r == FALSE)
		blink_error(12);

	sectors_left_in_multiblock -= 1;
	if (sectors_left_in_multiblock == 0) {
		r = MDD_SDSPI_MultiWriteFinish();
		if (r == FALSE)
			blink_error(13);
	}
}
...
data_run(void)
{
	...
	lba = 775;	// set the start address

#ifdef SHORT_ERASE
	sectors_before_erase = 0;
#endif
#ifdef MULTIBLOCK_LEN
	multi_init();
#endif
