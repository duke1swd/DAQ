/* This version works.  SPI loops are fully unrolled.
 * The A/D code is running at speed.  The MMC code
 * is also running at speed.
 */
/*
 * Firmware for the small A/D converter board.
 *
 * This is the simplified version.  It supports one channel
 * and has a target sample rate of 1 KHz.
 *
...
#define	TRISA_SET	0b.0001.0000	/* RA4 is the only port A input. */
#define	TRISB_SET	0b.0111.0000	/* RB4, 5, 6 are inputs */
...
#pragma bit RECORD_BUTTON @ PORTB.5	// input control

/*
 * Control for Timer 1.
 */
#define	T1PD		(10000L/1L)	// number of FOSC/4 cycles for 1000 HZ
...
#define	ERROR_USR_2		2	// record switch set w/out record button
...
#ifdef SMALLRING
#define	RING_SIZE	4
uns16 ring[RING_SIZE*2];		// ring buffer, the access bank

#else

#define	RING_SIZE	32		// number of entries in ring buffer.
#pragma rambank 0
uns16 ring[RING_SIZE*2];		// ring buffer, not in the access bank
#pragma rambank -
#endif
...
adc_sample()
{
	...
	value1 = 0;	// this goes here for timing reasons
	...
	ADC_RX_BIT(value1.12);
	ADC_RX_BIT(value1.11);
	ADC_RX_BIT(value1.10);
	ADC_RX_BIT(value1.9);
	ADC_RX_BIT(value1.8);
	ADC_RX_BIT(value1.7);
	ADC_RX_BIT(value1.6);
	...
	display_value = value1; // if you remove this, put the nop2() calls back in.
	// nop2();
	// nop2();

	/* 
	 * ADC_SAMPLE_1 0b.1110.0111;
	 */

	ADC_SEL = 0;
	value2 = 0;	// this goes here for timing reasons
	...
	ADC_RX_BIT(value2.12);
	ADC_RX_BIT(value2.11);
	ADC_RX_BIT(value2.10);
	ADC_RX_BIT(value2.9);
	ADC_RX_BIT(value2.8);
	ADC_RX_BIT(value2.7);
	ADC_RX_BIT(value2.6);
...
/*
 * Test the ADC driver by pulling 0.25 seconds of data out of the ring
 * buffer.
 */
void
adc_test()
...
unsigned char led_on;		// units are 10 ms. 
unsigned char led_off;		// units are 10 ms.
 ...
 * Status is led_on ms on, followed by led_off ms off, repeat
 */
void
led_control()
{
	/*
	 * There are 10 interrupts per millisecond.
	 */
	if (led_divider < 10) {
		led_divider++;
		return;
...
/*
 * During normal operations the 8-bit display value
 * is clocked out at 1 KHz onto pin 0.
 */
void
status_bit()
{
...
main()
{
	...
	while (RECORD_SWITCH == 0)
		;
#ifdef notdef // not during breadboard testing
	if (RECORD_BUTTON == 0)
		set_error(ERROR_USR_2);
#endif
	...
	led_off = 100;
	for (;;)
		if (RECORD_BUTTON == 1 || RECORD_SWITCH == 1)
