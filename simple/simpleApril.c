 ...
 * Configure the chip.
 */
#pragma config[1] = 0b.0000.0110	//disable switchover mode
					//disable fail-safe clock monitor
					//set HS+PLL oscillator mode
#pragma config[2] = 0b.0000.1101	//disable brown-out detect
					//disable power up timer
#pragma config[3] = 0b.0001.1110	//disable watchdog timer
#pragma config[5] = 0b.0000.0000	//disable master-clear
#pragma config[6] = 0b.1000.0000	//disable debugging
					//disable low voltage programming
					//disable stack overflow reset
...
#define	T1SET_H		((-T1PD + 19L) >> 8)
#define	T1SET_L		((-T1PD + 19L) & 0xff)
...
#define	RING_SIZE	40		// number of entries in ring buffer.
uns16 ring[RING_SIZE*2] @ 0x40;		// ring buffer, not in the access bank
...
ring_pop2()
{
	...
	/*
	 * This works because ring_n-- is a single atomic instruction.
	 */
	ring_n--;
...
adc_sample()
{
	...
	value1 = file_number;	// this goes here for timing reasons
	...
	ADC_RX_BIT(extra);	// discard the first bit
	ADC_RX_BIT(value1.15);
	ADC_RX_BIT(value1.14);
	ADC_RX_BIT(value1.13);
	...
	ADC_SEL = 0;
	/* Set the low bit on all samples. */
	value2 = 1;	// this goes here for timing reasons
	if (DIGITAL_INPUT)
		value2.3 = 1;

	/*
	 * Note: value2.1 and value2.2 are unused, always 0.
	 */
	...
	ADC_RX_BIT(extra);	// discard the first bit
	ADC_RX_BIT(extra);	// discard the first bit
	ADC_RX_BIT(value2.15);
	ADC_RX_BIT(value2.14);
	ADC_RX_BIT(value2.13);
...
timer1_interrupt()
{
	if (TMR0L.0)
		nop2();
	/*
	 * We get here exactly every 1250 instructions.
	 */
