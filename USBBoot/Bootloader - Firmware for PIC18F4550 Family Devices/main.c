...
void main(void)
{
    ...
    for (i = 1; i; i++) ;	// Wait a few ms for mode select cap to charge

    temp = ADCON1;
	TRISA = 1;
    ADCON1 = 0x0E;		// RA0/AN0 is analog
    ADCON0 = 0x00;		// ADC is disabled, set to channel 0.
    ADCON2 = 0x16;		// Aq time = 4 TAD, TAD = Fosc/64
    ADCON0 = 0x01;		// Enable ADC
    ADCON0bits.GO_DONE = 1;	// Start conversion
    while (ADCON0bits.GO_DONE)	// Wait for conversion
    	;

    if (ADRESH) {		// Any non-zero value causes jump
