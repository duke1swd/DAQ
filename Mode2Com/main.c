...
//
// Modified 7/22/8 by Stephen Daniel
// Modified to support the Hybridsky data collection project.
//
...
#elif defined(HYBRIDSKY)

   //
   // No configuration pragmas are defined here.
   // This program is loaded by a bootloader and 
   // inherits chip configuration from the boat loader.
   //
   // There are two possible bootloaders (USB and SD Memory),
   // but both use the same configuration.
   //
   // The config pragmas a shown here for reference,
   // but they are commented out so that they don't show up
   // in the hex file.  The reason for this is that
   // in the event of a mismatch the goal is to protecdt
   // the bootloader, not the bootloaded.
   //
...
// #pragma config CPUDIV   = OSC1_PLL2
// #pragma config USBDIV   = 2       // Clock source from 96MHz PLL/2
// #pragma config FOSC     = HSPLL_HS // this should give us an internal clock speed of 48 MHz
// #pragma config FCMEN    = OFF
// #pragma config IESO     = OFF
// #pragma config PWRT     = OFF
// #pragma config BOR      = ON
...
#pragma code _RESET_INTERRUPT_VECTOR_2 = 0x001000
void _reset_2 (void)
{
    _asm goto _startup _endasm
}
