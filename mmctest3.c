...
#define	ABORT_WRITE_FAILED 7 //didn't get back a success token after a write block data phase.
 ...
 */
#define mmc_spi_tx_bit(b) MMC_SCK = 0; MMC_SDO = b; MMC_SCK = 1
...
#define	MMC_WRITE_BLOCK		(0x40 | 24) // MMC write one block
#define	MMC_WRITE_BLOCK_TOKEN	0xfe	// mark the start of a WRITE_BLOCK data section
#define	MMC_WRITE_SUCCESS 0b.0010.1000 // OK response to a write.
...
/*
 * Write n sectors, 512b each, containing the constant byte code
 */
void
mmc_write_blocks(uns16 n)
{
	unsigned char r;
	unsigned char i;
	int addr_low:16, addr_high:16;

	MMC_CS = 0;	// Enable
	addr_low = 0;
	addr_high = 0;
	for ( ; n > 0; n--) {
		r = mmc_command(MMC_WRITE_BLOCK, addr_high, addr_low);		
		if (r != 0)
			abort2(ABORT_WR_BLK, r);
		mmc_spi_tx(MMC_WRITE_BLOCK_TOKEN);
